...
doInstall() {
  ...
  confirmInstall "LogCat"

  action "Copied script to /usr/bib directory" \
         "cp" "SOURCES/logcat" "/usr/bin/logcat"

  action "Added +x flag for script" \
         "chmod" "+x" "/usr/bin/logcat"

  if [[ ! -e /etc/logcat.conf ]] ; then
    action "Copied config file to /etc directory"
           "cp" "SOURCES/logcat.conf" "/etc/logcat.conf"
  fi

  action "Changed permissions for config file" \
         "chmod" "644" "/etc/logcat.conf"
